package org.celestialcraft.cCUtilities.modules.customparticles;

import org.bukkit.Material;
import org.bukkit.event.entity.EntityDamageByEntityEvent;
import org.bukkit.event.player.PlayerItemHeldEvent;
import org.bukkit.event.player.PlayerJoinEvent;
import org.bukkit.event.player.PlayerQuitEvent;
import org.bukkit.inventory.ItemStack;
import org.celestialcraft.cCUtilities.modules.customenchants.CustomEnchant;
import org.celestialcraft.cCUtilities.util.EnchantUtil;
import org.celestialcraft.cCUtilities.util.LoreUtil;

public class BloodyIcosphereEnchant implements CustomEnchant {

    private static final String RAW_LORE = "&7Bloody Icosphere Particles";

    @Override
    public String getIdentifier() {
        return "bloody_icosphere_particles";
    }

    @Override
    public String getLoreLine() {
        return RAW_LORE;
    }

    @Override
    public boolean appliesTo(ItemStack item) {
        return item != null && item.getType() != Material.AIR;
    }

    @Override
    public boolean hasEnchant(ItemStack item) {
        return EnchantUtil.hasTag(item, getIdentifier()) || LoreUtil.itemHasLore(item, RAW_LORE);
    }

    @Override
    public void applyEffect(EntityDamageByEntityEvent event) {
        // No combat effect; only particle visuals
    }

    @Override
    public ItemStack applyTo(ItemStack item) {
        if (item == null || !appliesTo(item) || hasEnchant(item)) return item;
        LoreUtil.ensureLoreAtTop(item, RAW_LORE);
        EnchantUtil.setTag(item, getIdentifier());
        return item;
    }

    // Central listener handles equip/unequip; keep hooks as no-ops for interface compatibility
    @Override
    public void onHeld(PlayerItemHeldEvent event) { /* no-op */ }

    @Override
    public void onJoin(PlayerJoinEvent event) { /* no-op */ }

    @Override
    public void onQuit(PlayerQuitEvent event) { /* no-op */ }
}
